...
def extract_name_smart(text: str) -> Optional[str]:
    ...
    # Get first 800 characters (header section of resume) - increased for better coverage
    ...
    # Try to find name in first 5-7 lines (most resumes have name at top)
    for i, line in enumerate(lines[:7]):
        ...
        # Skip lines that are too long (likely descriptions) - but allow up to 60 chars for longer names
        if len(line) > 60:
            continue
        
        # Skip lines that are too short (likely initials or single words)
        if len(line) < 4:
            continue
        ...
        # Pattern 2: First substantial line with 2-5 capitalized words (likely name)
            ...
            if capitalized_count >= len(words) * 0.8:  # At least 80% capitalized
                ...
                if not re.search(r'\d', line.replace('.', '')):
                    ...
                    if not any(char in line for char in ['@', '#', '$', '%', '&', '*', '(', ')']):
                        ...
                        # Final check: not a false positive
                        if (potential_name_lower not in false_positives and 
                            not any(fp in potential_name_lower for fp in false_positives)):
                            # #region agent log
                            try:
                                with open(log_path, 'a', encoding='utf-8') as log_file:
                                    log_file.write(json.dumps({
            ...
            for ent in doc.ents:
                if ent.label_ == 'PERSON':
                    ...
                    if (name_lower not in false_positives and 
                        len(name_candidate.split()) <= 5 and
                        len(name_candidate) > 2 and
                        not any(fp in name_lower for fp in false_positives)):
...
def extract_ner(text: str):
    """Extract college and location using NER"""
    college = None
    location = None
